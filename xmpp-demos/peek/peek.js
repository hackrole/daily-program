var Peek = {
  ...
  show_tracffic: function (body, type){
  ...
  pretty_xml: function(xml, level){
    ...
    for(i=0; i<attrs.length; i++){
      ...
      if (i != attrs.length -1){
        result.push("</div><div class='xml_elvel1" + level + "'>");
...
$(document).bind('connect', function(ev, data){
  var conn = new Strophe.Connection("http://localhost:8801/http-bind");
  conn.xmlInput = function(body){
    Peek.show_traffic(body, 'incoming');
  };

  conn.xmlOutput = function(body){
    Peek.show_traffic(body, 'outging');
  };
